...
BoardFrame::BoardFrame(const wxString& title) : wxFrame(nullptr, wxID_ANY, title)
{
	...
	for (int i = 0; i < board->getY(); i++) {
		for (int j = 0; j < board->getX(); j++) {
			...
			auto boundDrawLine = std::bind(&BoardFrame::drawLine, this, std::placeholders::_1, &(board->current_x), &(board->current_y), j, i, board);
...
void BoardFrame::drawLine(wxCommandEvent& event, int* current_x, int* current_y, int destiny_x, int destiny_y, Board* board) 
{
		wxClientDC dc(this);
...
void BoardFrame::switchCurrentField(int* current_x, int* current_y, int destiny_x, int destiny_y, Board* board)
{
